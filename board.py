...
class Board:
    ...
    def applyPlacement(self, tetromino, xPos, rotation):
        tetromino.rotateCoords(rotation)
        minY = tetromino.getMinYCoord()
        if minY < 0:
            tetromino.incrementCoords(0, -minY)
        self.moveFarLeft(tetromino)
        self.moveOrLockPiece(tetromino, Direction.RIGHT, xPos)
        self.dropPieceWithoutLock(tetromino)
        linesBefore = self.linesCleared
        self.moveLeftAndLockPiece(tetromino, 2)
        linesCleared = self.linesCleared - linesBefore
